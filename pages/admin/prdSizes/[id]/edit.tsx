...
// import { getprdSize, updateprdSize } from "../../../../redux/prdSizeSlice";
...
const EditprdSize: NextPageWithLayout = (props: Props) => {
  ...
  const onSubmit: SubmitHandler<Inputs> = async (values: Inputs) => {
    try {
      // await dispatch(updateprdSize(values)).unwrap();
      toast.success("Cập nhật  thành công");
      router.push("/admin/prdSizes");
    } catch (error) {
      console.log(error);
    }
  };
  useEffect(() => {
    (async () => {
      try {
        // const user = await dispatch(getprdSize(id)).unwrap();
        // reset(user);
        ...
        <form action="" method="POST" onSubmit={handleSubmit(onSubmit)}>
          <div className="shadow overflow-hidden sm:rounded-md">
            <div className="px-4 py-5 bg-white sm:p-6">
              <span className="font-semibold mb-4 block text-xl">Sửa Kích Cỡ:</span>
...
EditprdSize.getLayout = (page: ReactElement) => <AdminLayout>{page}</AdminLayout>;

export default EditprdSize;
